...
interface TerminalState {
  ...
  updateWorkspace: (id: string, updates: Partial<{ title: string; color: string }>) => void
...
export const useTerminalStore = create<TerminalState>()((set, get) => ({
  ...
  updateWorkspace: (id, updates) => {
    set((state) => {
      const workspace = buildWorkspaces(state.sessions, state.groups).find(w => w.id === id)
      if (!workspace) return state
      ...
      if (workspace.isGroup) {
        return {
          groups: state.groups.map((g) =>
            g.id === id
              ? { ...g, name: updates.title ?? g.name, color: updates.color ?? g.color }
              : g
          ),
        }
      } else {
        return {
          sessions: state.sessions.map((s) =>
            s.id === id
              ? { ...s, title: updates.title ?? s.title, color: updates.color ?? s.color }
              : s
          ),
        }
      }
