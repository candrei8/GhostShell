...
import { Theme, Provider, SavedAgentConfig, AgentAvatarConfig, LaunchPreset, GridLayout } from '../lib/types'
...
interface SettingsState {
  ...
  addLaunchPreset: (preset: LaunchPreset) => void
...
const VALID_LAYOUTS = new Set(['1x1', '1x2', '2x2', '3x2', '4x2', '5x2', '4x3', '5x3', '4x4'])

function sanitizeLaunchPreset(value: unknown): LaunchPreset | null {
  if (!value || typeof value !== 'object') return null
  const raw = value as Record<string, unknown>

  const id = pickTrimmedString(raw.id, '')
  const name = pickTrimmedString(raw.name, '')
  if (!id || !name) return null

  const layout = typeof raw.layout === 'string' && VALID_LAYOUTS.has(raw.layout)
    ? raw.layout as GridLayout
    : '1x1'

  const rawCounts = raw.providerCounts && typeof raw.providerCounts === 'object'
    ? raw.providerCounts as Record<string, unknown>
    : {}
  const providerCounts: Record<Provider, number> = {
    claude: pickNumber(rawCounts.claude, 0, 0, 9),
    codex: pickNumber(rawCounts.codex, 0, 0, 9),
    gemini: pickNumber(rawCounts.gemini, 0, 0, 9),
  }
  ...
  return {
    ...
    layout,
    providerCounts,
    yolo: typeof raw.yolo === 'boolean' ? raw.yolo : false,
    createdAt: pickNumber(raw.createdAt, Date.now(), 0, Number.MAX_SAFE_INTEGER),
  }
}

function sanitizeLaunchPresets(value: unknown): LaunchPreset[] {
  if (!Array.isArray(value)) return []
  const seen = new Set<string>()
  const out: LaunchPreset[] = []
  for (const entry of value) {
    const sanitized = sanitizeLaunchPreset(entry)
    if (!sanitized || seen.has(sanitized.id)) continue
    seen.add(sanitized.id)
    out.push(sanitized)
  }
...
function normalizePersistedSettings(persistedState: unknown) {
  ...
  return {
    ...
    launchPresets: sanitizeLaunchPresets(raw.launchPresets),
  }
}
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set, get) => ({
      ...
      addLaunchPreset: (preset) =>
        set((state) => {
          const sanitized = sanitizeLaunchPreset(preset)
          if (!sanitized) return state
          return { launchPresets: [...state.launchPresets, sanitized] }
        }),
      removeLaunchPreset: (id) => set((state) => ({ launchPresets: state.launchPresets.filter((p) => p.id !== id) })),
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        launchPresets: state.launchPresets,
      }),
