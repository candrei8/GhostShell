...
import { MoreHorizontal, RotateCw, Send, ChevronDown } from 'lucide-react'
...
export function OrchestratorSection({ agent }: OrchestratorSectionProps) {
  ...
  const tokenPct = metrics && metrics.maxTokens > 0
    ? Math.min(100, Math.round((metrics.tokenEstimate / metrics.maxTokens) * 100))
    : 0
  ...
  return (
    <div className="flex flex-col px-3 pt-2 pb-1">
      {/* 1. Agent Header — click to switch terminal */}
      <div
        className="flex items-center gap-2 h-8 cursor-pointer group"
        onClick={handleClick}
      >
        {/* Status dot */}
        <span
        ...
        {/* Agent name */}
        <span className="text-xs font-semibold text-ghost-text truncate flex-1 min-w-0">
          {agent.name}
        </span>
      ...
      {/* 2. Context Stats Bar */}
      {metrics && metrics.turnCount > 0 && (
        <div className="flex items-center gap-2 h-5 ml-3.5">
          {/* Mini token bar */}
      ...
      {activeSubAgents.length > 0 && (
        <div className="flex flex-col gap-1 mt-1.5 ml-1 mc-tree-line">
