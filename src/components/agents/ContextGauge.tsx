  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2" title={`Context: ${formatTokens(metrics.tokenEstimate)} / ${formatTokens(metrics.maxTokens)} tokens`}>
        ...
        <span className="text-xs text-ghost-text-dim font-mono tabular-nums">{pct}%</span>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        <span className="text-xs text-ghost-text-dim uppercase tracking-wider font-medium">Context</span>
        <span className="text-xs text-ghost-text-dim font-mono tabular-nums">
